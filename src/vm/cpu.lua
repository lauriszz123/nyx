...
function CPU:step()
	...
	elseif op == 0x72 then    -- CMP A, #imm8
		...
		self:flag(FLAGS.V, (bit.band(self.A, 0x80) ~= bit.band(imm, 0x80)) and (bit.band(result, 0x80) ~= bit.band(self.A, 0x80)))
		return 2
	elseif op == 0x73 then    -- CMP A, B
		...
		self:flag(FLAGS.N, bit.band(result, 0x80) ~= 0)
		self:flag(FLAGS.V, (bit.band(self.A, 0x80) ~= bit.band(self.B, 0x80)) and (bit.band(result, 0x80) ~= bit.band(self.A, 0x80)))
		return 1
	...
	elseif op == 0xFF then   -- HALT
		error('HALT instruction encountered')
    else                     -- NOP / undefined
        return 1
    end
end
